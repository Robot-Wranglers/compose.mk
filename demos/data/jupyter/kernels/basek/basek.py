        ...
        with tempfile.NamedTemporaryFile(dir=None, prefix=".tmp.ipy.cmk") as tmpfile:
            ...
            try:
                ...
                if result.returncode != 0:
                    output += stderr
                if not silent:
                    resp = self.iopub_socket, "stream", {"name": "stdout", "text": output}
                    self.send_response(*resp)
                if result.returncode != 0:
                    self.log.critical(f"stderr: {stderr}")
                    return dict(
                        status="error", traceback=[],ename="failed", 
                        execution_count=self.execution_count,evalue=f"{result.returncode}")
                else:
                    os.remove(fname)
                    return dict(status="ok", user_expressions={},payload=[], execution_count=self.execution_count,)
            except Exception as e:
                resp = self.iopub_socket, "stream", {"name": "stderr", "text": str(e)}
                if not silent: self.send_response(*resp)
                return dict(status="error", evalue=str(e),execution_count=self.execution_count,ename=type(e).__name__, traceback=[],)
