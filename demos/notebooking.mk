...
#
# See the main docs:
# https://robot-wranglers.github.io/compose.mk/demos/notebooking
#
...
## Next section uses the service scaffolding to create target-handles for
## language kernels dynamically.  Although `compose.import` already created
## handles for all the containers involved, we want to plan for being able
## to import from other compose files, or use future targets as kernels
## directly.  This means we want carefully designed namespaces.
## Kernel-invocation also involves accepting a filename as argument.  Thus
## for each service, we map a new convenience target to an existing
## scaffold: `kernel.<svc>/<fname>` --> `fmtk/<svc>.command/<fname>`
##░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
...
## Next section generates kernel.json files for each of the "kernel.*"
## targets above, regardless of whether those targets are static or
## dynamic.  This step involves a template for jupyter kernelspec JSON,
## which is our starting place for generating dynamic kernels on the
## jupyter side.  This works by deferring most of the kernel behaviour to a
## base class (i.e.  `BaseK`), and basically adjusts environment variables
## to configure kernel-names, kernel-commands, etc.. just in time.  For
## more details, see the appendix with support code/notebooks in the main
## docs.
##░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
...
## Next section is a small bridge to the jupyter lab HTTP API.  This isn't
## necessarily that useful since we have CLI access to jupyter.. but it
## shows it's accessible and that calls to `curl` could be replaced with
## `nbclient`, etc.
##░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░

api.kernels:
	@# Show kernel status, according to the web ui
	${io.curl} -s "${lab.url.base}/api/kernels" | ${jq} .

api.kernels.busy:
	@# Shows only busy kernels
	${io.curl} -s "${lab.url.base}/api/kernels" \
		| ${jq} '.[] | select(.execution_state=="busy")'

api.sessions:
	@# Show sessions
	${io.curl} -s "${lab.url.base}/api/sessions" | ${jq} .
...
lab.notebook.preview/%:
	...
	&& $(call log, $${header}( ${bold.underline}input${no_ansi})) \
	&& ${make} lab.dispatch.quiet/self.notebook.preview.in/${*} \
		| ${stream.markdown} \
	&& $(call log, $${header}( ${bold.underline}output${no_ansi})) \
	...
	&& cat $${tmpf} | grep '!\[png\]' > /dev/null\
	  && ($(call log, Detected image(s). Attempting preview..) \
...
lab.notebook.imgcount/%:
	...
	cat ${*} | ${jq} -r '${jq.imgcount.filter}'

lab.notebook.preview.img/%:
	$(call log, ${dim_cyan}Image #${bold}${cyan}$${i}\n)
...
lab.summary: lab.wait lab.test
	...
	$(call log, ${dim_cyan}Available notebooks:)
	${make} lab.notebooks | ${stream.indent}
	$(call log, ${dim_cyan}Markdown notebooks:)
...
self.kernelspec.list:
	...
	$(call log, ${dim_cyan}Available Kernels:)
...
self.notebooks.normalize:
	...
	$(call log, Pairing and syncing all markdown notebooks with ipynbs)
...
self.notebook.preview.in/%:
	@# Show markdown from ipynb, pre-execution.
	...
	&& $(call log, Notebook has $${lines} lines) \
	&& [ "$${lines}" -lt 90 ] \
	&& (\
