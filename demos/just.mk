...
#
# just.mk: Some interoperability with `just`.
#
# USAGE: ./demos/just.mk
...
# A small container-api, considered "private". Targets below run inside the
# container, so we can directly use `just` here without assuming it is
...
# Using a dispatch alias, you can "lift" a private target to a "public"
# one.  This target is safe to run from the docker host, and abstracts away
# the container.
just.list: just_container.dispatch/self.just.list

# Stacking 2 layers of dispatch is a bridge!  Effectively importing all the
# justfile targets to top-level `make`. Now you can then expose the full
# underlying tool API, or an opinionated subset, or create an API superset
# at this layer that extends it.
...
# The bridge also allows us to compose brand new functionality really
# quickly.
#
# As an example, below we provide an interactive runner for the justfile
# that lets us choose which target to kick off.  To do this, we can use the
