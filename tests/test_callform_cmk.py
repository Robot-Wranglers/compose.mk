...
cases drive the standalone `mk.preprocess.callform` target so the nonzero exit and
...
def test_target_bare_paren_is_args(cmk):
  r = _recipe(cmk, "this.foo(a,b)")
  assert "${make} foo/a,b" in r.stdout


def test_target_args_whitespace_stripped(cmk):
  # STRIP_WS applies to args only.
  r = _recipe(cmk, "this.foo( a , b )")
  assert "${make} foo/a,b" in r.stdout


def test_target_paren_then_bracket(cmk):
  r = _recipe(cmk, f"this.foo(a,b)[{TQ}S{TQ}]")
  assert "printf '%s' \"S\" | ${make} foo/a,b" in r.stdout


def test_target_bracket_then_paren(cmk):
  # swapped order -- identical lowering.
  r = _recipe(cmk, f"this.foo[{TQ}S{TQ}](a,b)")
  assert "printf '%s' \"S\" | ${make} foo/a,b" in r.stdout


def test_target_bracket_only(cmk):
  r = _recipe(cmk, f"this.foo[{TQ}S{TQ}]")
  assert "printf '%s' \"S\" | ${make} foo" in r.stdout


def test_target_glyph_stream(cmk):
  # a bare ⬦ref can't be a pipe LHS -> `cat ` prefixed before blockref lowers it.
  r = cmk("mk.compile", stdin="define d\nhi\nendef\nx:\n\tthis.foo[⬦d]\n")
  assert r.ok, r.stderr
...
def test_target_tagged_literal(cmk):
  r = _recipe(cmk, f"this.foo{SQ}L{SQ}")
  assert "printf '%s' 'L' | ${make} foo" in r.stdout


def test_target_tagged_interpolating(cmk):
  r = _recipe(cmk, f"this.foo{TQ}${{V}}{TQ}")
...
def test_drop_target_paren_is_args(cmk):
...
def test_macro_bare_paren_verbatim_to_call(cmk):
  # callform leaves `cmk.NAME(args)` verbatim; the late .awk.cmk.call lowers it.
  r = _recipe(cmk, "cmk.foo(a,b)")
  assert "$(call foo,a,b)" in r.stdout


def test_macro_paren_then_bracket(cmk):
  r = _recipe(cmk, f"cmk.foo(a,b)[{TQ}S{TQ}]")
  assert "printf '%s' \"S\" | $(call foo,a,b)" in r.stdout


def test_macro_bracket_then_paren(cmk):
  # swapped order -- identical lowering.
  r = _recipe(cmk, f"cmk.foo[{TQ}S{TQ}](a,b)")
  assert "printf '%s' \"S\" | $(call foo,a,b)" in r.stdout


def test_macro_bracket_only(cmk):
  r = _recipe(cmk, f"cmk.foo[{TQ}S{TQ}]")
  assert "printf '%s' \"S\" | $(call foo)" in r.stdout


def test_macro_literal_single_line(cmk):
  r = _recipe(cmk, f"cmk.foo[{SQ}1 2 3 4{SQ}]")
  assert "printf '%s' '1 2 3 4' | $(call foo)" in r.stdout


def test_macro_args_may_contain_brackets(cmk):
  # (args) is balanced-paren scanned, so `[`/`]` inside it are ordinary chars.
  r = _recipe(cmk, f"cmk.f(x[0],y)[{TQ}B{TQ}]")
  assert "printf '%s' \"B\" | $(call f,x[0],y)" in r.stdout


def test_macro_glyph_stream(cmk):
  r = cmk("mk.compile", stdin="define d\nhi\nendef\nx:\n\tcmk.h[⬦d]\n")
  assert r.ok, r.stderr
  assert "cat <($(call _mk.def.to.fd, d)) | $(call h)" in r.stdout


def test_macro_glyph_file_stream(cmk):
...
def test_macro_unquoted_target_body(cmk):
  # a `this.X` stream body: dialect lowers it to ${make} X before callform pipes it.
  r = _recipe(cmk, "cmk.h[this.t]")
  assert "${make} t | $(call h)" in r.stdout


def test_macro_unquoted_non_command_not_a_compile_error(cmk):
...
def test_macro_multiline_bracket_on_own_line(cmk):
  r = cmk("mk.compile", stdin=f"x:\n\tcmk.h[{SQ}L1\nL2{SQ}\n]\n")
  assert r.ok, r.stderr
...
def test_two_macros_one_line(cmk):
  r = _recipe(cmk, f"cmk.a(p)[{TQ}S{TQ}] cmk.b(q)")
...
def test_target_space_opts_out_of_callform(cmk):
...
def test_tagged_inert_inside_define(cmk):
  r = cmk("mk.compile", stdin=f"define blk\nthis.foo{SQ}L{SQ}\nendef\n")
  assert r.ok, r.stderr
...
# These drive the standalone `mk.preprocess.callform` target, so the macro anchor is the
...
def test_error_unterminated_unquoted_stream(cmk):
  r = cmk("mk.preprocess.callform", stdin=f"x:\n\t{SENT}h[a b\n")
  assert not r.ok
...
def test_nested_target_three_levels(cmk):
  r = _recipe(cmk, "this.a[this.b[this.c]]")
  assert "${make} c | ${make} b | ${make} a" in r.stdout


def test_nested_target_four_levels(cmk):
  r = _recipe(cmk, "this.a[this.b[this.c[this.d]]]")
  assert "${make} d | ${make} c | ${make} b | ${make} a" in r.stdout


def test_nested_macro_outer_target_inner(cmk):
  # outer macro `a` consumes the inner target pipeline as its stdin.
  r = _recipe(cmk, "cmk.a[this.b[this.c]]")
  assert "${make} c | ${make} b | $(call a)" in r.stdout


def test_nested_target_outer_macro_stream_inner(cmk):
  # inner macro `b` consumes target `c` as its stream, all inside target `a`'s stream.
  r = _recipe(cmk, "this.a[cmk.b[this.c]]")
  assert "${make} c | $(call b) | ${make} a" in r.stdout


def test_nested_with_outer_args(cmk):
  # the outer target carries (args) AND a nested stream -- both must survive.
  r = _recipe(cmk, "this.a(p)[this.b[this.c]]")
...
def test_nested_preserves_inner_literal(cmk):
