...
the runtime assembles the harness wiring (generic mk.repl.kernel eval for `true`; read/eval/print for the
object form), and it degrades cleanly with no tty.  The live TUI (a pty/pyte driver) is verified manually.
...
def test_repl_true_pragma_is_zero_boilerplate(tmp_path):
  ...
  assert "eval=mk.repl.kernel" in out, out  # generic target-namespace dispatcher
  assert "no tty so skipping tux.repl" in out, out  # headless degrade
...
def test_repl_object_pragma_wires_regions(tmp_path):
  ...
  assert rc == 0, out
  assert "entering REPL execution mode" in out, out
  for frag in ("eval=my.eval", "read=my.read", "print=my.print", "exit_after=1"):
...
def test_cmk_repl_no_file_drops_to_core_shell():
  ...
  assert "eval=mk.repl.kernel" in out, out
  assert "no tty so skipping tux.repl" in out, out
  assert "No rule to make target" not in out, out  # the old bare-`cmk repl` failure
...
def test_cmk_repl_subcommand_forces_mode_on_any_file(tmp_path):
  ...
  rc, out = _run("repl", f)
  assert rc == 0, out
  assert "eval=mk.repl.kernel" in out, out  # generic dispatcher, no pragma needed
  assert "no tty so skipping tux.repl" in out, out
