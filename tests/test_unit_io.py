...
@pytest.mark.unit
def test_io_bash_stdin(cmk, tmp_path):
  # io.bash runs a named define-block as a bash script; stdin is piped through.
  mk = tmp_path / "wrap.mk"
  mk.write_text(f"include {COMPOSE_MK}\ndefine echo_sh\ncat /dev/stdin\nendef\n")
  r = cmk("io.bash/echo_sh", stdin="foo bar", makefile=mk)
  assert r.ok, r.stderr
  assert "foo bar" in r.stdout


@pytest.mark.unit
def test_io_bash_args(cmk, tmp_path):
  # io.bash proxies positional args after the def name: io.bash/<def>,<a>,<b>.
  mk = tmp_path / "wrap.mk"
  mk.write_text(f"include {COMPOSE_MK}\ndefine greet\necho hello ${{1}} ${{2}}\nendef\n")
  r = cmk("io.bash/greet,foo,bar", makefile=mk)
  assert r.ok, r.stderr
  assert "hello foo bar" in r.stdout


@pytest.mark.unit
def test_io_bash_propagates_failure(cmk, tmp_path):
  # a non-zero exit inside the block propagates as the target's exit status.
  mk = tmp_path / "wrap.mk"
  mk.write_text(f"include {COMPOSE_MK}\ndefine boom\nexit 3\nendef\n")
  r = cmk("io.bash/boom", makefile=mk)
  assert not r.ok
