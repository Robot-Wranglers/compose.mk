...
def test_flux_bg_backgrounds_and_reaps(cmk, tmp_path):
  # flux.bg runs a target in the background (control returns before it finishes) and
  # flux.bg.wait blocks until it completes -- so the job's marker is absent right after
  # flux.bg and present only after flux.bg.wait.  It is an inline recipe macro (the bg
  # job lives in the caller's shell), so this drives it through a small `cmk run` file.
  src = tmp_path / "bg.cmk"
  src.write_text(
    "import flux\n"
    "slow:; sleep 1; echo DONE > marker\n"
    "probe:\n"
    "\trm -f marker; \\\n"
    "\tflux.bg(slow); \\\n"
    "\t[ -f marker ] && echo EARLY=yes || echo EARLY=no; \\\n"
    "\tflux.bg.wait(slow); \\\n"
    "\t[ -f marker ] && echo REAPED=yes || echo REAPED=no\n"
    "__main__: probe\n"
  )
  r = cmk("cmk", "run", str(src), cwd=tmp_path, env={"CMK_SUPERVISOR": "1"}, timeout=60)
  out = r.stdout + r.stderr
  assert r.ok, out
  assert "EARLY=no" in out    # backgrounded: the job had not finished yet
  assert "REAPED=yes" in out  # flux.bg.wait blocked until it did


def test_flux_retry_polls_until_success(cmk, tmp_path):
  # flux.retry re-runs a command until it exits 0 (bounded).  Here the probe `[ -f flag ]`
  # is false until a backgrounded job creates the flag ~0.5s later, so flux.retry blocks
  # through several attempts and then succeeds.
  src = tmp_path / "retry.cmk"
  src.write_text(
    "import flux\n"
    "slow:; sleep 0.5; touch flag\n"
    "probe:\n"
    "\trm -f flag; \\\n"
    "\tflux.bg(slow); \\\n"
    "\t$(call flux.retry, [ -f flag ]); \\\n"
    "\t[ -f flag ] && echo RETRY=ok || echo RETRY=timedout; \\\n"
    "\tflux.bg.wait(slow)\n"
    "__main__: probe\n"
  )
  r = cmk("cmk", "run", str(src), cwd=tmp_path, env={"CMK_SUPERVISOR": "1"}, timeout=60)
