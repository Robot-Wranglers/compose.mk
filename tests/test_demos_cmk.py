...
their own source sections from `/dev/stdin` (`mk.fork.section`), which has no
...
ALL_DEMOS = sorted(p.name for p in DEMOS_DIR.glob("*.mk"))

# Demos that consume their OWN source via `mk.fork.section` (reading GUEST /
...
SELF_REFERENTIAL = {
  "fault.mk": "forks its own /dev/stdin GUEST section (mk.fork.section)",
