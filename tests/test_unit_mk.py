...
lang.comp.pipeline.*, and mk.parse golden output) are intentionally NOT covered here
...
def test_hook_rewrite_skips_compiler_and_interpreter(cmk):
  # mk.compile / lang.comp.pipeline / mk.interpret pass through UNwrapped (no hooks).
  for target in ("mk.compile", "lang.comp.pipeline", "mk.interpret"):
...
# --- cli.subcommands engine: robust tail capture (subcommands.tail) -----------
# `subcommands.tail` recovers a dispatcher's CLI tail from `${MAKE_CLI}`: a sed drops the
# make-invocation prefix up to & incl. `mk.super.enter/<pid>`, then make drops the flux.pre/*
# flux.post/* hook decorations and the leading namespace token. Unit-tested in isolation via a
# wrapper makefile that includes compose.mk and overrides MAKE_CLI (no supervisor needed).

_ANCHOR = (
  "make -sS --warn-undefined-variables -f ./compose.mk mk.super.enter/9"
)


def _tail(cmk, tmp_path, goals: str) -> str:
  w = tmp_path / "tail.mk"
  w.write_text(
    f"include {COMPOSE_MK}\noverride MAKE_CLI := {_ANCHOR} {goals}\n"
    "_probe:;@echo '$(subcommands.tail)'\n"
  )
  r = cmk("_probe", makefile=str(w))
  assert r.ok, r.stderr
  return r.stdout.strip()


def test_subcommands_tail_decorated(cmk, tmp_path):
  ...
  assert _tail(cmk, tmp_path, goals) == "run foo.cmk"


def test_subcommands_tail_undecorated(cmk, tmp_path):
  # hooks-off (the test env) or a skip-listed namespace: no decoration.
  assert _tail(cmk, tmp_path, "cmk run foo.cmk") == "run foo.cmk"


def test_subcommands_tail_path_arg(cmk, tmp_path):
  # slash-bearing args are never decorated by the rewrite; pass through intact.
  assert _tail(cmk, tmp_path, "cmk run demos/cmk/x.cmk") == "run demos/cmk/x.cmk"


def test_subcommands_tail_empty(cmk, tmp_path):
  # namespace with no args -> empty tail (engine then prints usage).
  assert _tail(cmk, tmp_path, "cmk") == ""
...
def test_subcommands_usage_lists_reflected_subs(cmk, tmp_path):
  w = _wrapper(tmp_path, _SUBCMD_WRAPPER)
  r = cmk(
    "cli.subcommands", makefile=w, env=_denv("help", default="", subs="")
  )
  assert r.ok, r.stderr
...
def test_mk_require_tool_memo_survives_recursive_make(cmk, tmp_path):
  # The success memo is an EXPORTED var, so a recursive ${make} inherits it and
  # skips the re-probe: a composed target that fans out to a sub-make probes the
  # tool ONCE, not once per process (the gitops.stat regression).
  w = _wrapper(
    tmp_path,
    "memo.outer: assert.tool.required/bash\n"
    "\t@${make} memo.inner\n"
    "memo.inner: assert.tool.required/bash\n"
    "\t@true\n",
  )
  r = cmk("memo.outer", makefile=w)
  assert r.ok, r.stderr
  assert r.stderr.count("looking for") == 1, r.stderr
...
def test_mk_def_dispatch(cmk, tmp_path):
  # Runs an interpreter on a define-block (here sh runs the script). Shares its
  # recipe with the same-line target host.dispatch.
...
def test_super_stderr_filter_dims_cascade_and_denoises(cmk):
  # The supervisor stderr filter, as a testable target (same .awk.super.stderr.* blocks the
  # bootloader source-reads): dims the `make[N]: *** [..] Error M` propagation cascade,
  # collapses an external-Ctrl-C `*** [..] Interrupt` cascade to one clean warning,
  # passes normal lines through, splits a run-together `<content>make: *** [` line, and
  # range-deletes the SIGINT self-kill noise.
  DIM = "  \x1b[2m"  # cascade lines are indented 2 spaces + dimmed
  # dim + passthrough
  r = cmk("mk.super.stderr.filter", stdin="make[4]: *** [x] Error 2\nhello\n")
  assert r.ok, r.stderr
  assert DIM + "make[4]: *** [x] Error 2" in r.stdout   # cascade indented + dimmed
  assert "\nhello\n" in ("\n" + r.stdout)               # normal line untouched
  # split a run-together line into content + (dimmed) make error
  r2 = cmk("mk.super.stderr.filter", stdin="log outputmake: *** [y] Error 1\n")
  assert "log output" in r2.stdout and DIM + "make: *** [y]" in r2.stdout
  # SIGINT self-kill noise range is deleted (range ends on a top-level `make: *** Error`)
  noise = "make[1]: *** [t] mk.interrupt/SIGINT ... Killed\nmake: *** Error 2\nkeep me\n"
  r3 = cmk("mk.super.stderr.filter", stdin=noise)
  assert "mk.interrupt/SIGINT" not in r3.stdout and "keep me" in r3.stdout
  # an external-Ctrl-C `*** [..] Interrupt` cascade collapses to ONE clean warning (not dimmed)
  cascade = "make[2]: *** [a] Interrupt\nmake[1]: *** [b] Interrupt\nmake: *** [.tux.init] Interrupt\n"
  r4 = cmk("mk.super.stderr.filter", stdin=cascade)
  assert "Interrupt" not in r4.stdout             # raw cascade lines suppressed
  assert r4.stdout.count("interrupted") == 1      # exactly one warn for the whole cascade


# --- io.curl dual-use + the m5.__splat__ helper (no network) ----------------

_IOCURL_PROBE = (
  "_probe:\n"
  "\t@printf 'BARE=[%s]\\n' '$(io.curl)'\n"
  "\t@printf 'SPLAT1=[%s]\\n' '$(call io.curl,-s \"http://x/api/kernels\")'\n"
  "\t@printf 'SPLATN=[%s]\\n' '$(call io.curl,-s,-o,/dev/null,http://x)'\n"
  "\t@printf 'HELPER=[%s]\\n' '$(call m5.__splat__,a,b,c)'\n"
)


def test_io_curl_backcompat_and_variadic_splat(cmk, tmp_path):
  # io.curl is dual-use: a bare `${io.curl}` is JUST the curl command (no trailing args --
  # back-compat), while a callform `$(call io.curl, ..)` splats its (comma-separated) args,
  # unquoted, after the command.  Purely inspects the EXPANSION -- curl is never run, no network.
  w = _wrapper(tmp_path, _IOCURL_PROBE)
  r = cmk("_probe", makefile=w)
  assert r.ok, r.stderr
  out = r.stdout
  # bare: the command alone -- no request flag / url leaked in (works whether curl is the host
  # binary or the dockerized fallback)
  bare = next(l for l in out.splitlines() if l.startswith("BARE="))
  assert "-s" not in bare and "http" not in bare, bare
  # one callform arg: spaces + quotes preserved verbatim
  assert '-s "http://x/api/kernels"' in out, out
  # many comma args: splatted, space-joined, single-spaced (no $(foreach) join artifacts)
  assert "-s -o /dev/null http://x" in out, out
  # the extracted helper on its own
  assert "HELPER=[a b c]" in out, out
