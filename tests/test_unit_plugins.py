...
def test_import_plugin_lenient_missing_tolerated(cmk, tmp_path):
  ...
  body = "$(call include.plugin, file=nope.mk strict=0)\nprobe:; @echo CONTINUED\n"
  r = cmk("probe", makefile=_wrapper(tmp_path, body))
  assert r.ok, r.stderr
...
def test_include_plugin_cmk_lenient_missing_tolerated(cmk, tmp_path):
  ...
  body = "$(call include.plugin, file=nope.cmk strict=0)\nprobe:; @echo CONTINUED\n"
  r = cmk("probe", makefile=_wrapper(tmp_path, body))
  assert r.ok, r.stderr
  assert "CONTINUED" in r.stdout
...
def test_require_skips_when_registry_inherited(cmk, tmp_path):
  ...
  r = cmk("probe", env={"__plugins__": "ho.mk"}, makefile=_wrapper(tmp_path, body))
  assert r.ok, r.stderr
  assert "HO=[]" in r.stdout  # require saw the inherited entry -> SKIPPED (not imported)
...
def test_module_require_skips_when_registry_inherited(cmk, tmp_path):
  ...
  assert rh.ok and "HMV=[mod-loaded]" in rh.stdout, rh.stderr  # first require -> imported
  rc = cmk("probe", env={"__modules__": "hm"}, makefile=_wrapper(tmp_path, body))
