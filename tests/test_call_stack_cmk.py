...
pytestmark = [pytest.mark.unit, pytest.mark.covers_demo("call_stack.cmk")]
...
def test_goto_back_loop():
  # `cmk.vm.goto(count/..)` -- a bounded loop over the control stack.
...
def test_call_return_resumes_continuation():
  # `cmk.vm.call(probe/..)` + `cmk.vm.return()`; the saved continuation [C] resumes.
