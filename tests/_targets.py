...
def _def_line_groups(compose_mk: Path):
  ...
  for line in Path(compose_mk).read_text().splitlines():
    if re.match(r"^define\s", line):
      ...
      # embedded Dockerfile/awk/heredoc bodies this skip exists to ignore. Scan it.
      in_define = not re.match(r"^define\s+__hosted__(\s|$)", line)
      continue
...
# compose.import / docker.import / code synthesize whole target
...
# _docker.import, emit only fully-dynamic ${kwargs_def}-named targets,
# which aren't stable templates.)
_GENERATOR_DEFINES = {
